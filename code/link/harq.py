# -*- coding: utf-8 -*-
"""
HARQ Management for NR.

Provides:
- Simple HARQ manager with ACK delay and process gating
...
from .olla import create_olla_from_config
from .eesm import eff_sinr_eesm_db, combine_eff_sinr_db
from .adaptation import choose_mcs_from_sinr
from .bler import bler_awgn_sigmoid
from .mcs import MCS
from .tbs import calc_tbs_bits


class HarqManager:
    """Simple HARQ ACK deferral manager.
    ...
    - All transmissions ACK after the delay (no NACK/retransmission modeling)
    """
    ...
    def __init__(self, num_ue: int, num_procs: int, ack_delay_ttis: int):
        ...
        Args:
            ...
            num_procs: Number of HARQ processes per UE
            ack_delay_ttis: ACK delay in TTIs
        """
        self.N = int(num_ue)
    ...
    def advance_time(self, t: Optional[int] = None) -> None:
        ...
        Args:
            t: Target time (if None, increment by 1)
        """
        if t is None:
            self.t += 1
        else:
    ...
    def can_schedule(self, ue: int) -> bool:
        """Check if UE has available HARQ processes.
    ...
    def on_scheduled(self, scheduled_ues: np.ndarray) -> None:
        """Mark scheduled UEs for this TTI (unique set).

        Increases outstanding count and schedules completion.

        Args:
            scheduled_ues: Array of scheduled UE indices
        """
...
class HarqManagerFull:
    ...
    def __init__(
        ...
        """Initialize full HARQ manager.

        Args:
            num_ue: Number of UEs
            num_procs: Number of HARQ processes per UE
            ack_delay_ttis: ACK delay in TTIs
            config: Configuration dict with HARQ and link adaptation parameters
        """
        self.N = int(num_ue)
        self.P = max(1, int(num_procs))
        self.D = max(1, int(ack_delay_ttis))
        ...
        self._outstanding = np.zeros(self.N, dtype=int)
        # Per-UE OLLA
        self._olla = [create_olla_from_config(config, u) for u in range(self.N)]
    ...
    def is_full(self) -> bool:
        """Check if this is a full HARQ manager (vs simple)."""
        return True

    def advance_time(self, t: Optional[int] = None) -> np.ndarray:
        """Advance time and materialize ACK/NACK.

        Args:
            t: Target time (if None, increment by 1)

        Returns:
            Acked bits per UE for this TTI
        """
        if t is None:
            self.t += 1
        else:
    ...
    def on_scheduled_blocks(self, info: Dict[int, Dict]) -> None:
        ...
        Args:
            info: Dict mapping UE index to scheduling info:
                {
        ...
        for u, d in info.items():
            ...
            if retx_entry is not None:
                ...
                sinr_eff_now_db = eff_sinr_eesm_db(
                    ...
                    beta_db=float(d.get('eesm_beta_db', 1.0))
                )
    ...
    def _acquire_proc(self, ue: int) -> int:
        """Acquire a free HARQ process for UE.

        Args:
            ue: UE index

        Returns:
            Process index, or -1 if none available
        """
    ...
    def _decide_ack(self, ue: int, tb: Dict) -> bool:
        """Stochastic ACK/NACK decision based on BLER curve.
    ...
    def get_retx_requirements(self) -> Dict[int, Tuple[int, int]]:
        """Return desired PRB block (li,ri) for UEs pending retransmission.

        Used to keep resource consistency across retransmissions.

        Returns:
            Dict mapping UE index to (li, ri) tuple
        """
    ...
    def get_stats(self) -> Dict:
        ...
        Returns:
            Dict with ACK/NACK counts, MCS distribution, OLLA history, etc.
        """
