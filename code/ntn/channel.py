...
class NTNChannelProfile:
    ...
    Attributes:
        additional_loss_db: Per-state additional path loss (dB).
        shadow_sigma_db: Per-state shadow fading std deviation (dB).
        k_factor_db: Per-state Rician K-factor (dB). Use -inf for Rayleigh.
        los_logistic: Logistic parameters (slope, mid-point) for LoS probability.
        nlos_logistic: Logistic parameters for NLoS probability.
    """
...
NTN_CHANNEL_PROFILES: Dict[str, NTNChannelProfile] = {
    "s_band_handheld_urban": NTNChannelProfile(
        ...
        # Logistic fits tuned to TR 38.811 Annex 6 (50% LoS @ ~35 deg, rapid NLoS drop past 20 deg).
...
def sample_3gpp_ntn_fading(
    ...
    * ``states`` is an array of strings ``{"los", "slos", "nlos"}`` for
    ...
    Args:
        ...
        elevation_deg: Per-UE elevation angles in degrees.
        num_prb: Number of PRBs for small-scale fading.
        profile_name: Name of channel profile to use.
        overrides: Optional dict to override profile parameters.

    Returns:
        Tuple of (large_scale_db, fading_lin, states).
    ...
    for state in ("los", "slos", "nlos"):
        ...
        fading_lin[mask] = _rician_power_samples(
            rng,
            (np.count_nonzero(mask), num_prb),
            float(k_db.get(state, -np.inf))
        )
...
def describe_profile(profile_name: str) -> Dict[str, Dict[str, float]]:
    ...
    Args:
        profile_name: Name of the channel profile.

    Returns:
        Dictionary containing all profile parameters.
